...
function DuckSchema.getRarity(name: string): Rarity?
	for _, r in ipairs(DuckSchema.Rarities) do if r.name == name then return r end end
end
function DuckSchema.findOption(dim: string, name: string): DimOption?
	for _, o in ipairs(DuckSchema.Dimensions[dim] or {}) do if o.name == name then return o end end
end
function DuckSchema.forgeTierForPoints(points: number): ForgeTier
