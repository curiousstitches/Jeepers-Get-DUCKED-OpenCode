...
ZoneConfig.ZoneLength = 110
ZoneConfig.ZoneWidth  = 120
ZoneConfig.StartZ     = 0
...
function ZoneConfig.worldForZone(zone: number): World
	return ZoneConfig.Worlds[math.clamp(math.floor((zone-1)/10)+1, 1, #ZoneConfig.Worlds)]
end
...
function ZoneConfig.artForZone(zone: number): WorldArt
	return ZoneConfig.Art[math.clamp(math.floor((zone-1)/10)+1,1,10)]
end

function ZoneConfig.zoneOriginZ(zone: number): number
	return ZoneConfig.StartZ + (zone-1)*ZoneConfig.ZoneLength
end

function ZoneConfig.stats(zone: number): ZoneStats
	return {
		unlockCost = math.floor(300 * (1.18 ^ (zone-1))),
...
function ZoneConfig.currencyIds(): { { id: string, name: string, accent: Color3 } }
	local t = {}
