...
local danceBtn = bigBtn(playPage, "💃 DANCE PARTY", Color3.fromRGB(200, 100, 200), function()
	DanceParty.toggle()
end)
local danceStatus = infoLabel(playPage, "")
...
function MenuRefreshPlay()
	...
	local cd = DanceParty.cooldownRemaining()
	local rem = DanceParty.timeRemaining()
	if DanceParty.active then
		danceStatus.Text = "💃 Party time! " .. string.format("%.0fs", rem) .. " remaining — double loot!"
		danceBtn.BackgroundColor3 = Color3.fromRGB(255, 100, 200)
		danceBtn.Text = "💃 DANCE PARTY (" .. string.format("%.0fs", rem) .. ")"
	elseif cd > 0 then
		danceStatus.Text = "Party cooldown: " .. string.format("%.0fs", cd)
		danceBtn.BackgroundColor3 = Color3.fromRGB(100, 70, 100)
		danceBtn.Text = "⏳ COOLDOWN (" .. string.format("%.0fs", cd) .. ")"
	else
		danceStatus.Text = "Start a dance party — ducks wobble + double loot for 30s!"
		danceBtn.BackgroundColor3 = Color3.fromRGB(200, 100, 200)
		danceBtn.Text = "💃 DANCE PARTY"
	end
end
