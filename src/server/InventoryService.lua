...
local InventoryChanged: RemoteEvent?
local EquipDuck: RemoteEvent?

function InventoryService.MaxEquipped(player: Player): number
...
function InventoryService.SquadStrength(player: Player): number
...
function InventoryService.EnchantBonuses(player: Player): (number, number)
...
function InventoryService.AddDuck(player: Player, duck: any): boolean
...
function InventoryService.RemoveDucks(player: Player, idSet: { [string]: boolean })
...
local function isEquipped(p: PlayerData.Profile, id: string): number?
	for i, v in ipairs(p.equipped) do if v == id then return i end end
end
local function owns(p: PlayerData.Profile, id: string): boolean
...
function InventoryService.Unequip(player: Player, id: string): boolean
	local p = PlayerData.Get(player); if not p then return false end
...
function InventoryService.Start()
	...
	EquipDuck.OnServerEvent:Connect(function(player: Player, action: string, id: string)
