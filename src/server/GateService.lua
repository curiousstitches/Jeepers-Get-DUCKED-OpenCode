...
local Proximity          = require(game:GetService("ReplicatedStorage"):WaitForChild("Shared"):WaitForChild("Proximity"))
...
local function tp(player: Player, pos: Vector3)
	local char = player.Character
	local root = char and char:FindFirstChild("HumanoidRootPart")
	if root then root.CFrame = CFrame.new(pos) end
end

local function onGate(gate: BasePart, hit: BasePart?)
	local char = hit and hit.Parent
	local player = char and Players:GetPlayerFromCharacter(char)
	if not player then return end
	if not Proximity.requireNear(player, gate, 30) then return end
	...
	if (p[highestKey] or 1) >= level then return end
	...
	if CurrencyService.Spend(player, gateCurrency, cost) then
		...
		if Notify then Notify:FireClient(player, { text = ("Level %d unlocked! (%s)"):format(level, mode), color = Color3.fromRGB(80, 230, 120) }) end
	else
...
local function onVIP(pad: BasePart, hit: BasePart?)
	local char = hit and hit.Parent
	local player = char and Players:GetPlayerFromCharacter(char)
	if not player then return end
	if not Proximity.requireNear(player, pad, 20) then return end
	local key = player.UserId .. "_v"
	if debounce[key] then return end
	debounce[key] = true; task.delay(1.5, function() debounce[key] = nil end)
	local p = PlayerData.Get(player); if not p then return end
	if p._vip then
		tp(player, Vector3.new(-150, 268, -550))
	else
...
function GateService.Start()
	...
	local LaunchService = require(script.Parent.LaunchService)
	local function onShrine(shrine: BasePart, hit: BasePart?)
		local player = Players:GetPlayerFromCharacter(hit and hit.Parent); if not player then return end
		if not Proximity.requireNear(player, shrine, 20) then return end
