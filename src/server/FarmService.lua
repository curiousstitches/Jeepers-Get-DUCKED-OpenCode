...
local COMBO_WINDOW = 3.5 -- sec before combo resets

local state = {} -- [part] = { hp, max, reward, currency, dmg = {[userId]=n}, alive, t0 }
local combos = {} -- [userId] = { count, last }, combo streak tracking
local BreakSmashed: RemoteEvent? -- set in Start()
...
local function rewardTop(inst, s)
	...
	if prof then
		...
		if prof.mode == "hardcore" then mult = mult * 2.5 end
	end
	mult = mult * DanceService.GetMultiplier()
	local reward = math.max(1, math.floor(s.reward * mult))
	CurrencyService.Add(p, s.currency, reward)
	if prof and prof.stats then prof.stats.breakablesSmashed = (prof.stats.breakablesSmashed or 0) + 1 end
	-- combo tracking
	local c = combos[topId]
	local now = os.clock()
	if c and now - c.last <= COMBO_WINDOW then
		c.count += 1
	else
		c = { count = 1, last = now }
	end
	c.last = now
	combos[topId] = c
	-- fire visual event to client
	if BreakSmashed then
		BreakSmashed:FireClient(p, {
...
function FarmService.Start()
	BreakSmashed = Remotes.event("BreakSmashed")
