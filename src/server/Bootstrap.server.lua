...
local DanceService        = require(script.Parent.DanceService)
...
local DebugCommands       = require(script.Parent.DebugCommands)
...
safeStart("DanceService", DanceService.Start)
...
safeStart("DebugCommands", DebugCommands.Start)
...
-- signal all clients that the game is ready (loading screen dismissal)
local Remotes = require(script.Parent.Remotes)
local GameReady = Remotes.event("GameReady")
task.wait(0.5)
for _, p in ipairs(Players:GetPlayers()) do
	local pd = PlayerData.Get(p)
	if pd then GameReady:FireClient(p) end
end
Players.PlayerAdded:Connect(function(p)
	task.spawn(function()
		local tries = 0
		while not PlayerData.Get(p) and tries < 50 do task.wait(0.1); tries += 1 end
		if PlayerData.Get(p) then GameReady:FireClient(p) end
	end)
end)
...
local function teleportToLobby(char)
	local hrp = char:FindFirstChild("HumanoidRootPart") or char:WaitForChild("HumanoidRootPart", 5)
	if hrp and SkyLobby.Spawn then
		hrp.CFrame = CFrame.new(SkyLobby.Spawn)
	end
end
...
Players.PlayerAdded:Connect(function(player)
	task.spawn(function()
		...
		if player.Character then teleportToLobby(player.Character) end
	end)
end)
for _, p in ipairs(Players:GetPlayers()) do
	task.spawn(ensureStarter, p)
	if p.Character then teleportToLobby(p.Character) end
	p.CharacterAdded:Connect(teleportToLobby)
end
